...
import oberon.content.traveling.Sailing;
import oberon.model.objects.Ladders;
import oberon.model.objects.Object;
import oberon.util.Misc;
import oberon.util.ScriptManager;
...
public class ActionHandler {
	...
	public void firstClickObject(int objectType, int obX, int obY) {
		...
		case 10284:
			...
			break;
		case 8143:
			c.getFarming().finishedHerbPatch(c.getFarming().choosePatch());
		break;
		//doors
		case 6749:
		...
		case 6823:
			if(oberon.content.minigames.Barrows.selectCoffin(c, objectType)) {
				return;
			}
    ...
    public void firstClickNpc(int i)
    {
        ...
        case 316:
            c.getFishing().setupFishing(c.getFishing().checkFish(317));//Shrimp
            if (c.playerLevel[10] >= 15) {
            	c.getFishing().setupFishing(c.getFishing().checkFish(319));//Anchovies
            }
            break;
            
        case 309:
        	c.getFishing().setupFishing(c.getFishing().checkFish(335));//Trout
            if (c.playerLevel[10] >= 30) {
            			c.getFishing().setupFishing(c.getFishing().checkFish(331));//Salmon
            	}
            break;
            
        case 312:
        	c.getFishing().setupFishing(c.getFishing().checkFish(377));
        	break;
        	
        case 313:
        	c.sendMessage("Click the second option.");
            break;
        ...
        case 599: 
            c.getDH().sendDialogues(29, i);
            break;
            
        case 2790:
			c.getDH().sendDialogues(35, i);
		break;	    
    ...
    public void secondClickNpc(int i)
    {
        ...
        case 316:
        	c.getFishing().setupFishing(c.getFishing().checkFish(327));//Sardine
        	if (c.playerLevel[10] >= 10) {
        		c.getFishing().setupFishing(c.getFishing().checkFish(345));//Herring
        	}
        	break;
            
        case 309:
        	c.getFishing().setupFishing(c.getFishing().checkFish(349));//Pike
            break;
            
        case 312:
        	c.getFishing().setupFishing(c.getFishing().checkFish(359));
        	if (c.playerLevel[10] >= 50) {
        		c.getFishing().setupFishing(c.getFishing().checkFish(371));
        	}
        	break;
        	
        case 313:
        	c.getFishing().setupFishing(c.getFishing().checkFish(383));
        	break;
