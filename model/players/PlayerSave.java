		...
		while(EndOfFile == false && line != null) {
			...
			if (spot > -1) {
				...
				case 2:
					...
					} else if (token.equals("difficulty-rank")) {
						p.difficultyRank = Integer.parseInt(token2);	
					} else if (token.equals("Difficulty")) {
						p.difficulty = Integer.parseInt(token2);	
					} else if (token.equals("first-time")) {
						p.firstTime = Integer.parseInt(token2);
					...
					} else if (token.startsWith("quest") && !token.contains("points")) {
						for (int i = 0; i < p.quest.length; i++) {
							if (token.equals("quest"+i)) {
							p.quest[i] = Integer.parseInt(token2);	
								}			
							}
	...
	**/
	public static boolean saveGame(Client p) {
		...
		try {
			...
			characterfile.newLine();
			characterfile.write("difficulty-rank = ", 0, 18);
			characterfile.write(Integer.toString(p.difficultyRank), 0, Integer.toString(p.difficultyRank).length());
			characterfile.newLine();
			characterfile.write("Difficulty = ", 0, 13);
			characterfile.write(Integer.toString(p.difficulty), 0, Integer.toString(p.difficulty).length());
			characterfile.newLine();
			...
			characterfile.newLine();
			for (int i = 0; i < p.quest.length; i++) {
				characterfile.write("quest"+i+" = ", 0, 9);
				characterfile.write(Integer.toString(p.quest[i]), 0, Integer.toString(p.quest[i]).length());
				characterfile.newLine();
			}
