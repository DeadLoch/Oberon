...
import oberon.content.traveling.GnomeGlider;
import oberon.model.items.GameItem;
import oberon.model.players.Client;
import oberon.model.players.PacketType;
import oberon.model.players.SkillMenu;
...
public class ClickingButtons implements PacketType {
	...
	public void processPacket(Client c, int packetType, int packetSize) {
		...
		case 118098:
			...
			break;
		case 62167:
			//add other cases of the action buttons in the quest tab
		//	c.getQH().handleDialogues(actionButtonId);
			break;
			...
			case 9190:
				...
				if (c.dialogueAction == 10) {
					...
					c.getPA().spellTeleport(3429, 3539, 0);
					c.dialogueAction = -1;
				} else if (c.dialogueAction == 20) {
					c.getPA().spellTeleport(2804, 10000, 0);
					c.dialogueAction = -1;	
				} else if (c.dialogueAction == 23)	{
					c.getPA().spellTeleport(3563, 3314, 0);
					c.dialogueAction = -1;
				}
				break;
				//mining - 3046,9779,0
			...
			case 9191:
				...
				if (c.dialogueAction == 10) {
					...
					c.getPA().spellTeleport(2884, 9800, 0);
					c.dialogueAction = -1;	
				} else if (c.dialogueAction == 20) {
					...
					if (c.difficulty == 2 || c.difficulty == 3) {
				c.nextChat = 26;
				} else {
				c.nextChat = 26;
				c.difficulty = 1;
				...
				c.sendMessage("You are now using the Rookie experience rate.");
			}
		}	
				break;
			//3rd tele option	
			...
			case 9192:
				...
				if (c.dialogueAction == 10) {
					...
					if (c.difficulty == 1 || c.difficulty == 3) {
						...
						c.difficultyRank = 2;
						c.getPA().closeAllWindows();
						c.sendMessage("You are now using the Knight experience rate.");
					}
				}	
				break;
			//4th tele option
			case 9193:
				...
				if (c.dialogueAction == 10) {
					...
					c.getPA().spellTeleport(3168, 9572, 0);
					c.dialogueAction = -1;
				} else if (c.dialogueAction == 23) {
					c.getPA().spellTeleport(3365, 3268, 0);
					c.dialogueAction = -1;
				} else if (c.dialogueAction == 28) {
					if (c.difficulty == 1 || c.difficulty == 2) {
						...
						c.difficultyRank = 3;
						c.getPA().closeAllWindows();
						c.sendMessage("You are now using the Elite experience rate.");
					}
				}
				break;
			//5th tele option
			case 9194:
				...
				if (c.dialogueAction == 10 || c.dialogueAction == 11) {
					...
					c.getDH().sendDialogues(34, 0);
				} else if (c.dialogueAction == 23) {
					c.getPA().closeAllWindows();
					c.sendMessage("Give us a suggestion for this teleport option.");
				}
				break;
		...
		case 9128: //Aggressive
			...
			case 125003://Accept
				...
				break;
			case 125006://Decline
				c.sendMessage("You have chosen to decline, Client will be disconnected from the oberon.");
				break;
