...
public class ClanChatHandler {
	...
	public void updateClanChat(int clanId) {
		for (int j = 0; j < clans[clanId].members.length; j++) {
			...
			if (Server.playerHandler.players[clans[clanId].members[j]] != null) {
				...
				c.getPA().sendFrame126("Talking in: @cc2@" + Misc.formatPlayerName(clans[clanId].name), 18139);
				c.getPA().sendFrame126("Owner: @whi@" + Misc.formatPlayerName(clans[clanId].owner), 18140);
				...
				for (int i = 0; i < clans[clanId].members.length; i++) {
					if (clans[clanId].members[i] > 0){
						if (Server.playerHandler.players[clans[clanId].members[i]] != null) {
							c.getPA().sendFrame126(Misc.formatPlayerName(Server.playerHandler.players[clans[clanId].members[i]].playerName), slotToFill);
							slotToFill++;
						}	
					}
				}
		...
		if (clans[clanId] != null) {
			for (int j = 0; j < clans[clanId].members.length; j++) {
				if (clans[clanId].members[j] <= 0) {
					...
					messageToClan(Misc.formatPlayerName(Server.playerHandler.players[playerId].playerName) + " has joined the channel.", clanId);
	...
	public void sendLootShareMessage(int clanId, String message) {
		if (clanId >= 0) {
			for (int j = 0; j < clans[clanId].members.length; j++) {
				...
				if (Server.playerHandler.players[clans[clanId].members[j]] != null) {
					Client c = (Client)Server.playerHandler.players[clans[clanId].members[j]];
					c.sendClan("Lootshare", Misc.formatPlayerName(message), Misc.formatPlayerName(clans[clanId].name), 2);
				}
			}
		}
	}
	...
	//	sendLootShareMessage(c.clanId, Misc.formatPlayerName(c.playerName) + " has received " + amount + "x " + oberon.model.items.Item.getItemName(item) + ". Price: "+ c.getPA().getTotalAmount(c, c.getShops().getItemShopValue(item) * amount)+".");	
	}
